...
GA_Sex<-c("M","F","M","F","M","F","M","F","M","F")
GA_Weight<-c(400,175,120,85,180,140,160,82,86,68)
GA_Height<-c(180,140,150,150,178,165,137,115,78,73)
GA_DF<-cbind.data.frame(GA_Species,GA_Sex,GA_Weight,GA_Height)
```
...
col_extract1<-GA_DF[,1:3] #in python this would include only columns 1 and 2
print(col_extract1)

```

```{r}
col_extract2<-GA_DF[,4]
print(col_extract2)
```
...
```{r}
#a slice that combines two criteria
# Only males and only individuals who weight more than 175 
...
```{r}
GA_DF$Lifespan<-c(43,43,55,55,82,85,58,58,57,57)
GA_DF
```
...
```{r}
#names(GA_DF)[,5]<-"GA_Lifespan"
#GA_DF
names(GA_DF)
# maybe we want to change the column name
colnames(GA_DF)[5]<-"GA_Lifespan"
```
...
```{r}
#read.csv()
write.csv(GA_DF, file="/Users/presgd/MyRFiles/Intro_to_R_2026_QMD/R1_Module1/GA_DF_new_column.csv")
