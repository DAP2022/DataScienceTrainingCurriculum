...
1.	Test for a difference between host species in cuckoo egg size. Include all four steps, including testing and investigating all ANOVA assumptions (bartlett.test() for homogeneity of variance, visual normality check).
2.	Using TukeyHSD(), identify which pairs of host species differ significantly. If part (a) failed to reject H₀, still run TukeyHSD() and comment on whether its conclusions are consistent with your ANOVA result.
...
1. Plot a boxplot that includes each of the three groups (not separate boxplots, all three groups should be on the same boxplot). Do these data match the assumptions of an ANOVA?
2. Do an appropriate test to ask whether lifespan differs between the three groups of flies. (Hint: use a nonparametric test in this case – don't worry about transformations since you have already worked through the transformations in the notebook and they are seldom worth the time....).
