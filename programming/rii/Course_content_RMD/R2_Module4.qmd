...
- We refreshed the Hypothesis testing pipeline:
  ...
  six individuals are given a treatment and 6 are given a placebo and we want to test whether or not there is a true difference between them: $H_o: \mu_1 - \mu_2 = 0$ $H_A: \mu_1 - \mu_2 \neq 0$ The assumption for the two sample t test is that the variances are reasonably equal. If the variances are dramatically different, you should use the default Welch's approximate t-test to buffer against unequal variances.
  ...
  Two sample t test is *important*. Let's use our expr_data set to answer the question: ***Does treatment change expression in brain tissue?*** Within the brain tissue, there are 200 controls and 200 samples. We already saw, in R2_Module3, that the two conditions were both normally distributed (which makes sense, since we simulated them from a normal distribution in R2_Module1).

```{r}
# read in expr_data from file where we placed in Module 1 or from our Environment (if it is still present)

# Let's return to our simulated biological dataset
brain <- subset(
  expr_data,
  Tissue == "brain"
)
...
boxplot(
  ...
  data = brain,
  col = c("#56B4E9", "#E69F00"),
  main = "Brain Gene Expression: Control vs Treated",
  xlab = "Condition",
  ylab = "Expression"
)
```

***Question: Does the Boxplot suggest a difference between control and treated samples?***

[Step 1:]{.underline}

H₀: Mean expression is the same in control and treated brain samples.\
Hₐ: Mean expression differs between control and treated brain samples.

[Step 2: two sample t test]{.underline}

To answer this, we first must test assumptions (done in R2_Module3):

```{r}
# Check normality visually
qqnorm(
  brain$Expression[brain$Condition == "control"],
  main = "Brain Control"
)
qqline(
  brain$Expression[brain$Condition == "control"]
...
qqnorm(
  brain$Expression[brain$Condition == "treated"],
  main = "Brain Treated"
)
qqline(
  brain$Expression[brain$Condition == "treated"]
...
The assumptions are all met! So, now we proceed with the t-test:
...
[Step 3: p-value]{.underline}

```         
p-value = 2.987e-06
```

[Step 4: Conclusion]{.underline}

It is a bit surprising when you look at the boxplots, but with 200 data points in each of the two categories, we can detect even a small true difference!

$-0.7732186<\mu_{control}-\mu_{treated}<-0.3198260$
...
Revisiting expr_data: We know how much treatment effect that we simulated for our current dataset of 1200; it was 0.5 expression units. So we can see why a modest effect became detectable with **large samples.**

- what would happen if we only had 10 individuals per condition? Let's simulate that and compare it to the power of the 200 per condition dataset:

```{r}
  # tiny dataset - sampled from our big dataset
  brain_small <- rbind(
    subset(brain, Condition == "control")[1:10, ],
    subset(brain, Condition == "treated")[1:10, ]
  )
  # run t test with tiny dataset
  t.test(
    Expression ~ Condition,
    data = brain_small
  )
...
OOOOOH. The p-value is 0.3183 now so we FTR the null hypothesis, but when we had 200 samples from each condition x tissue, we rejected the null hypothesis!
...
```
 We are at a decision point, the data is not normally distributed. What are our options? 
 1. Transformation
 2. Nonparametric alternative
 3. t-test might still be robust depending on sample size and severity. 
 
Here, I use the paired t-test to demonstrate the formula, we will subsequently see what happens with transformations. 
```{r}
# but we will use it anyway to demonstrate the formula and then see what happens with transformations. 
...
We will see the non-parametric alternative to one-sample and paired t-test next...
...
Welch's t-test is most appropriate if we know that the standard deviations are definitely, say, \>3. However, Welch's approx t-test is the default output of the t.test() and you need to explicitly provide the argument var.equal=TRUE in order to not use it! Note the difference in the degrees of freedom in the output and think about why that is the case.
...
# 4. Kaplan-Meier Estimator and Survival Curve
