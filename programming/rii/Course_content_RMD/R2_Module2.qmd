...
1.  Tidyverse uses logic and layering.
2.  We will use two different types of data set:

- built-in data set: star wars
- expr_data dataframe that we simulated in R2_Module1

2.  ggplot2 similarly uses the grammar of visualization
...
We will also load our simulated expr_data set from R2_Module1.

```{r}
expr_data<-read.csv(file="your file path here - use file.choose() in Console")
head(expr_data)
glimpse(expr_data)
print("~~~~~~~~~~~~~~~")
# expr_data is ALREADY a dataframe, but we need to ensure that it is specifically a tibble so this is good practice
expr_tbl<-as_tibble(expr_data)
glimpse(expr_tbl)
```
...
With the expr_tbl, we want to keep only the treated samples

```{r}
treated_samples <- expr_tbl |>
  filter(Condition == "treated")

head(treated_samples)
```
...
For our expr_tbl, we might want to put the highest-expression sample first:

```{r}
high_to_low <- expr_tbl |>
...
Back to expr_tbl example:

```{r}
# count(): how many samples fall into each expression category?
expr_tbl |>
  count(ExprCat)

# we saw these same counts represented as tab3d and a mosaic plot in R2_Module1!
# it is so easy with tidyverse...
expr_tbl |>
  count(Tissue, Condition, ExprCat)
```
...
And, now with expr_tbl in comparison to tapply!

```{r}
expr_summary <- expr_tbl |>
...
#tapply(expr_data$Expression, list(expr_data$Tissue,expr_data$Condition),mean)
expr_tbl_tapply_comp<-expr_tbl |>
  group_by(Tissue, Condition) |>
  summarize(
    mean_expression = mean(Expression)
  )
expr_tbl_tapply_comp 
```
...
And one last time with the expr_data:

```{r}
highest_expression <- expr_tbl |>
  group_by(Tissue, Condition) |>
  slice_max(Expression, n = 2)
...
Once again, we will revisit the expr_tbl simulated dataset:
...
expr_tbl |>
  ...
  geom_boxplot() +
  ggtitle("Gene Expression by Tissue and Treatment")

# to remind you, when we used plain old boxplot function: 
#boxplot(Expression ~ Tissue + Condition,data = expr_data)
```
