...
1.  **General Hypothesis testing features**: The **four steps of hypothesis testing** and a quick mention of terms such as Null and Alternate hypotheses, type I and type II errors, p-values, false discovery rate of multiple testing, sensitivity, specificity. There is a document called **"R2_Module3_Hypothesis_Testing_Review.qmd"** that covers these definitions, as well as a [review pdf](https://github.com/DAP2022/DataScienceTrainingCurriculum/blob/main/programming/rii/Course_content_RMD/Statistics_Review_R2.pdf) here. These are concepts that were covered in Biostatistics I and Biostatistics II so these are superficial reviews, but might be necessary refreshers!
    ...
    - In addition, here are two flowchart websites that include [decisions](https://statsandr.com/blog/what-statistical-test-should-i-do/#flowchart) about [**WHAT**](https://onishlab.colostate.edu/wp-content/uploads/2019/07/which_test_flowchart.png) test and even [one](https://www.scribbr.com/statistics/statistical-tests/) that goes into more depth about the same decisions that I outline over the next three R Notebooks. This [one](https://martinctc.github.io/blog/common-statistical-tests-in-r-part-i/) includes R (& Python) code and one that includes code for [visualizing](https://www.data-to-viz.com/) the data.
...
3.  **The Normal Distribution**
    ...
    - A worked example with the simulated expr_data

4.  **A worked example**: Using Hypothesis testing on the bumpus Data set

5.  **AUC/ROC (Area-under-Curve/Receiver-Operator Curve)**: ways to operationalize specificity and sensitivity when considering multiple hypotheses
...
- We saw these in R1, Module 4 when we investigated scatterplots and histograms.
...
Remember how we used rnorm() when we simulated the six groups in our expr_data dataframe? **Note that we should not run qqnorm (or Shapiro-Wilk etc) on the full 1200 observations since we simulated 6 different populations within those 1200 observations: brain-control, brain-treatment, liver-control, liver-treatment, heart-control, heart-treatment which were each simulated as normal distributions, but with a different mean.** Instead, we might want to break it down and ask ***Is gene expression approximately normally distributed within each experimental group?***

```{r}
# Brain controls only
brain_control <- subset(
  expr_data,
  Tissue == "brain" & Condition == "control"
)

hist(brain_control$Expression)
qqnorm(brain_control$Expression)
qqline(brain_control$Expression, col = "red")
```
...
...and now a quick check-in with the expr_data set. We will run Shapiro-Wilk on each of the six groups (brain-control, brain-treatment, liver-control, liver-treatment etc). Here, we ask ***Is gene expression approximately normally distributed within each experimental group?***

```{r}
brain_control <- subset(expr_data,Tissue == "brain" & Condition == "control")

hist(brain_control$Expression,main = "Brain Control Expression",xlab = "Expression",col = "steelblue")

qqnorm(brain_control$Expression)
qqline(brain_control$Expression, col = "red")

shapiro.test(brain_control$Expression)
```

We could also graph a boxplot to show the difference between Control and Treatment in Brain (which will be useful when we run t-tests!):

```{r}
brain <- subset(expr_data,Tissue == "brain")

boxplot(
  Expression ~ Condition,
  data = brain,
  col = c("#56B4E9", "#E69F00"),
  main = "Brain Expression: Control vs Treated",
  ylab = "Expression"
)
```
...
Since the AUC value is between 0.5 (random chance) and 1 (perfect discrimination), these would both be considered good classifiers, but wfns would be preferred between the two.

[**CoC Question 4**]{.underline}
